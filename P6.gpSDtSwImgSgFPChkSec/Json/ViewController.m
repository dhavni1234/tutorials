...
- (void)viewDidLoad
{
    ...
    self.btnOutletJson.layer.cornerRadius = self.btnOutletJson.frame.size.height /2;
    self.btnOutletJson.layer.masksToBounds = YES;
    self.btnOutletJson.layer.borderWidth = 0;
...
- (IBAction)showJson:(id)sender {
    ...
    [manager POST:@"http://192.168.1.92/*/*/*/*" parameters:parameter success:^(AFHTTPRequestOperation *operation, NSDictionary * responseObject) {
        ...
        dict1 = [responseObject valueForKey:@"data"];
        NSLog(@"dict1 holds %@", dict1);
