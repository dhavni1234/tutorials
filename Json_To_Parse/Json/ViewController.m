...
- (IBAction)showJson:(id)sender {
    ...
    [manager POST:@"http://108.163.162.202:8080/semipro/users/login" parameters:parameter success:^(AFHTTPRequestOperation *operation, id responseObject) {
       ...
       NSDictionary *data = responseObject;
        //self.array = [[NSArray alloc]initWithObjects:data, nil];
        //NSLog(@"array data: %@", self.array);
    
       

       // NSArray *studentkeys = [NSArray arrayWithObjects:@"fullname", @"email", @"dob", @"title" ,nil];

        //NSDictionary *dictionary = [NSDictionary dictionaryWithObjects:responseObject forKeys:studentkeys];
        ...
        NSString *dob = [data objectForKey:@"dob"];
